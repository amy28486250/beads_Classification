# -*- coding: utf-8 -*-
"""Monochrome classifier.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wI4fEcavqmmjheuZwASA285Jg90k_bg3
"""
...
train_path = path + '/train10'
test_path = path + '/test6'

#改了
model_path = path + '/model/multi_mix_modal_ResNet_7v44_tf_2_10'
...
datagen_train = ImageDataGenerator(
        ...
        #channel_shift_range=0.01  # Randomly shift colors
    )
...
def ResNet34(input_shape=(80, 80, 3), classes=7):
    ...
    # Block 2
    x = convolutional_block(x, filters=[64, 64, 256], strides=(1, 1), block_name='conv_block2_0')
    x = identity_block(x, filters=[64, 64, 256], block_name='id_block2_1')
    x = identity_block(x, filters=[64, 64, 256], block_name='id_block2_2')

    # Block 3
    x = convolutional_block(x, filters=[128, 128, 512], strides=(2, 2), block_name='conv_block3_0')
    x = identity_block(x, filters=[128, 128, 512], block_name='id_block3_1')
    x = identity_block(x, filters=[128, 128, 512], block_name='id_block3_2')
    x = identity_block(x, filters=[128, 128, 512], block_name='id_block3_3')

    # Block 4
    x = convolutional_block(x, filters=[256, 256, 1024], strides=(2, 2), block_name='conv_block4_0')
    x = identity_block(x, filters=[256, 256, 1024], block_name='id_block4_1')
    x = identity_block(x, filters=[256, 256, 1024], block_name='id_block4_2')
    x = identity_block(x, filters=[256, 256, 1024], block_name='id_block4_3')
    x = identity_block(x, filters=[256, 256, 1024], block_name='id_block4_4')
    x = identity_block(x, filters=[256, 256, 1024], block_name='id_block4_5')

    x = Dropout(0.3)(x)

    # Block 5
    x = convolutional_block(x, filters=[512, 512, 2048], strides=(2, 2), block_name='conv_block5_0')
    x = identity_block(x, filters=[512, 512, 2048], block_name='id_block5_1')
    x = identity_block(x, filters=[512, 512, 2048], block_name='id_block5_2')
    ...
    #這應該是ResNet50...
# 使用 ResNet34 函數建立模型
model = ResNet34(input_shape=(80, 80, 3), classes=7)
model.summary()
